...
public interface IPostService
{
    ...
    Task<PostResponse> GetByIdAsync(Guid postId, string? endUserToken = null, CancellationToken cancellationToken = default);

    Task<PostResponse> CreateAsync(Guid boardId, CreatePostRequest request, CancellationToken cancellationToken = default);

    Task<PostResponse> UpdateStatusAsync(Guid postId, Domain.Enums.PostStatus newStatus, Guid adminUserId, CancellationToken cancellationToken = default);

    Task<IReadOnlyList<StatusHistoryEntry>> GetStatusHistoryAsync(Guid postId, CancellationToken cancellationToken = default);
}
